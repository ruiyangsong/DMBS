...
#
# This loop for submitting jobs that include psi-blast, hh-blits and their MSAs calculating and filtering for each mutation
#
PREPARED=""
...
#
# This loop for integrating pre-calculated sequence-based features for each mutation.
#
for MT in `echo $MUT | awk -F "" '{for (i=1;i<=NF;i++){print($i)}}'`;
do
    [ $MT = $WT ] && continue
